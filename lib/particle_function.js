...
var converter1 = new Converter({checkType: false});
var converter2 = new Converter({checkType: false});
var _ = require ("underscore");
var tmp_time_json, play1_json, play2_json;


converter1.fromFile("./public/test_csv_black1.csv",function(err,result){
    if (err){
        console.log(err);
    }else{
        // console.log(result);
        play1_json = _.groupBy(result, function(obj){ return obj.Time; });
        tmp_time_json = play1_json;
        // console.log(tmp_time_json);
    }
});

converter2.fromFile("./public/test_csv_black2.csv",function(err,result){
    if (err){
        console.log(err);
    }else{
        // console.log(result);
        play2_json = _.groupBy(result, function(obj){ return obj.Time; });
        // console.log(play2_json);
    }
});
...
exports.player_change = function (player_num) {
    return Promise.resolve(1).then(
        function onFulfilled(){
            if (current_player == player_num ){
                console.log("Same Player requested");
            }else {
                if (player_num == 1){
                    current_player = 1;
                    tmp_time_json = play1_json;
                    console.log("action switched to player 1");
                    // console.log(tmp_time_json);
                }else if (player_num == 2){
                    current_player = 2;
                    // console.log(play2_json);
                    tmp_time_json = play2_json;
                    console.log("action switched to player 2");
                    // console.log(tmp_time_json);
                }
            }
        }
